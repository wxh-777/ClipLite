...
- 渲染窗口改用 Direct2D 1.1 DeviceContext、D3D11 硬件设备和 DXGI swap chain，硬件创建失败时回退 WARP。
- 兼容旧版 Windows 10 的 D3D11 设备级别参数，11.1 参数不接受时自动重试 11.0/10.1。
- 性能测量脚本在已有单实例接管启动请求时明确报告采样无效，不再输出空进程的误导性指标。
