...
- Windows 11 历史窗口使用系统 DWM 材质；不支持时自动回退为普通主题背景，不进行桌面截图或持续模糊缓存。
...
- 安装包：`ClipLite-Setup-1.2.6-x64.exe`。安装向导支持 English 和简体中文，会按 Windows 界面语言自动选择；同时支持自定义安装目录、开始菜单/桌面快捷方式和标准卸载。
- 便携包：`ClipLite-1.2.6-portable-win-x64.zip`。解压后复制整个目录即可使用，数据保存在其中的 `data\\` 目录。
