...
- 安装包：`ClipLite-Setup-1.2.5-x64.exe`。安装向导支持 English 和简体中文，会按 Windows 界面语言自动选择；同时支持自定义安装目录、开始菜单/桌面快捷方式和标准卸载。
- 便携包：`ClipLite-1.2.5-portable-win-x64.zip`。解压后复制整个目录即可使用，数据保存在其中的 `data\\` 目录。
