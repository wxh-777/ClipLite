...
- 后台运行时，本机任务管理器观察到的内存约为 `2.4 MB`。
- 主程序约 `535 KB`，比一张普通手机照片还小。
...
当前 Windows x64 Release 本机实测：Working Set `13.35 MB`、Private Bytes `2.40 MB`、主程序 `535.5 KB`、启动耗时 `60.12 ms`、GDI `13`、USER `10`。任务管理器和测量工具的内存统计口径不同，因此显示数值可能不同。
...
| --- | --- | --- |
| 后台占用 | 轻量设计，本机空闲观察约 `2.4 MB` | 功能越多，常驻占用通常越高 |
| 程序体积 | 主程序约 `535 KB` | 依赖运行时或集成功能后通常更大 |
