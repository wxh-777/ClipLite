...
![ClipLite 历史窗口](https://raw.githubusercontent.com/wxh-777/ClipLite/v1.0.5/docs/screenshots/history.png)

设置窗口：

![ClipLite 设置窗口](https://raw.githubusercontent.com/wxh-777/ClipLite/v1.0.5/docs/screenshots/settings.png)
...
- 后台运行时，本机任务管理器观察到的内存约为 `2.7 MB`。
- 主程序约 `550 KB`，比一张普通手机照片还小。
...
当前 Windows x64 Release 本机实测：Working Set `13.51 MB`、Private Bytes `2.73 MB`、主程序 `549.5 KB`、启动耗时 `56.12 ms`、GDI `13`、USER `10`。任务管理器和测量工具的内存统计口径不同，因此显示数值可能不同。
...
| --- | --- | --- |
| 后台占用 | 轻量设计，本机空闲观察约 `2.7 MB` | 功能越多，常驻占用通常越高 |
| 程序体积 | 主程序约 `550 KB` | 依赖运行时或集成功能后通常更大 |
...
- 安装包：`ClipLite-Setup-1.0.5-x64.exe`。支持自定义安装目录、开始菜单/桌面快捷方式和标准卸载。
- 便携包：`ClipLite-1.0.5-portable-win-x64`。复制整个目录即可使用，数据保存在其中的 `data\\` 目录。
...
在设置页的历史窗口快捷键区域，可以开启“粘贴后移到首位”。开启后，历史项目会按最近使用活动排序；关闭时按最近复制时间排序。程序还会保存每条记录的稳定 ID、首次录入时间、毫秒级最近复制时间、最后使用时间、复制次数、使用次数和逻辑内容长度，为后续排序及筛选提供数据基础。
