...
构建目录规则：始终复用 `build-x64/`。如果 `build-x64\Release\ClipLite.exe` 或相关目标文件被占用，先确认占用进程对应本仓库的 ClipLite 实例，再按 PID 直接终止该进程后重新构建，即使该实例正在被用户使用；不得按进程名批量终止，也不得通过创建新的构建目录绕过文件占用。
...
- 截图测试使用的图片格式仅允许 JPEG、PNG、GIF 或 WebP；禁止生成、保存或提交其他格式的截图文件。
