...
- `src/clip_store.cpp`：本地历史存储实现；负责追加式二进制记录、v1/v2 读取、CRC 校验、截断恢复、删除、重建、搜索和 DPAPI 加密。
...
- `build-x64/`、`build/`、`out/`：CMake 构建和发布输出，不是源代码；由 `.gitignore` 排除，不要直接修改生成文件。
...
- 测试会修改当前用户的 `%LOCALAPPDATA%\ClipLite` 数据；测试结束应清理，避免影响开发者的真实剪贴板历史。
